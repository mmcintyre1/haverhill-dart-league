...
interface MatchMeta {
  ...
  awayTeamId: string;
  weekKey: string;      // DC "27 Jan 2026" format
...
// ── loadPostSeedGuids ─────────────────────────────────────────────────────────
// Returns match GUIDs already in the DB for this season's POST matches.
// Prevents losing data when DC's history API has a delay returning new GUIDs.

async function loadPostSeedGuids(
  seasonId: number,
  dcTeamToSerialId: Map<number, number>
  ...
  try {
    ...
    const existing = await db
      ...
      .from(matches)
      .where(and(eq(matches.seasonId, seasonId), eq(matches.seasonStatus, "POST")));
    for (const m of existing) {
      if (!m.dcGuid || result.has(m.dcGuid)) continue;
...
// ── scrapePhase ───────────────────────────────────────────────────────────────
// Single parameterized pass for both REG and POST seasons.
// Called twice per scrape: once with phase="REG" (empty seedGuids, full leaderboardMpr),
// once with phase="POST" (DB-seeded GUIDs, empty leaderboardMpr).

async function scrapePhase(
  targetSeasonId: number,
  phase: "REG" | "POST",
  teamCompetitors: Competitor[],
  dcTeamToSerialId: Map<number, number>,
  dateToRoundSeq: Map<string, number>,
  seedGuids: Map<string, MatchMeta>,
  g3: G3Config,
  ...
  // Merge seed GUIDs — won't overwrite GUIDs already found from history
  for (const [guid, meta] of seedGuids) {
    if (!matchMeta.has(guid)) matchMeta.set(guid, meta);
  }

  // Round inference: assign roundSeq to history matches using date offset from known lineup rounds
  {
  ...
  debug[`${phase}_segmentsLoaded`] = segmentsMap.size;
  ...
  for (const [guid, meta] of matchMeta) {
    ...
    const roundSeq: number | null =
      ...
      : matchData?.roundSeq != null ? matchData.roundSeq
  ...
  for (const p of roster) {
    ...
    if (phase === "REG") {
      ...
      await db
        ...
        .values({ playerId, seasonId: targetSeasonId, teamId: teamId ?? null, teamName: teamName || null, divisionId: teamDivisionId ?? null, divisionName })
        .onConflictDoUpdate({
          target: [playerSeasonTeams.playerId, playerSeasonTeams.seasonId],
          set: { teamId: teamId ?? null, teamName: teamName || null, divisionId: teamDivisionId ?? null, divisionName },
        });
    }
    ...
    if (phase === "REG" && acc && acc.opponentNames.length > 0) {
    ...
    const computedMpr = acc && acc.crktDartsTotal > 0
...
async function scrapeSeasonStats(
  ...
  // ── B. Team loading: try DC standings page → DB fallback ────────────────────
  // DC's standings page may return empty competitors mid-playoffs; fall back to
  // DB teams which are stable after the first successful scrape of the season.
  ...
  if (teamCompetitors.length === 0) {
    const dbTeams = await db
      .select({ dcId: teams.dcId, name: teams.name })
      .from(teams)
      .where(eq(teams.seasonId, targetSeasonId));
    teamCompetitors = dbTeams.map(t => ({ id: t.dcId, team_name: t.name, name: t.name }));
    debug.teamsSource = "db";
  } else {
  ...
  const g3: G3Config = {
    include180:     g3CfgMap["g3.include_180"]     !== "false",
    includeRo9:     g3CfgMap["g3.include_ro9"]     !== "false",
    includeHout:    g3CfgMap["g3.include_hout"]    !== "false",
    includeRo6b:    g3CfgMap["g3.include_ro6b"]    !== "false",
    include100p:    g3CfgMap["g3.include_100plus"] === "true",
    includeRnds:    g3CfgMap["g3.include_rnds"]    === "true",
    includeBulls:   g3CfgMap["g3.include_bulls"]   === "true",
    includePerfect: g3CfgMap["g3.include_perfect"] === "true",
  };
