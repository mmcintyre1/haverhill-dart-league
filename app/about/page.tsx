...
async function getSiteConfig(): Promise<{ contactEmail: string | null; aboutDescription: string }> {
  const rows = await db
    .select()
    .from(siteContent)
    .where(inArray(siteContent.key, ["contact.email", "about.description"]));
  const map = Object.fromEntries(rows.map((r) => [r.key, r.value]));
  return {
    contactEmail: map["contact.email"] ?? null,
    aboutDescription: map["about.description"] ?? DEFAULT_ABOUT,
  };
}

export default async function AboutPage() {
  const { contactEmail, aboutDescription } = await getSiteConfig();
    ...
    <div className="max-w-3xl space-y-10">
      ...
      <section>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900 px-6 py-5">
          <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{aboutDescription}</p>
        </div>
      </section>
