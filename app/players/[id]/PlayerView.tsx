...
// explicit /players/[id]/[seasonId] (dynamic) page.
export default async function PlayerView({
  ...
  activeId: number;
  phase: string;
  // Whether (activeId, phase) is the player's default view — when true,
  // the "back to leaderboard" link targets the bare, static /leaderboard
  // page instead of the dynamic /leaderboard/[seasonId].
  ...
  const leaderboardHref = isActiveSeason
    ? "/leaderboard"
    : `/leaderboard/${activeId}${phase !== "REG" ? `?phase=${phase}` : ""}`;

  return (
    <div>
      {/* Back link */}
      <div className="mb-5">
        <Link href={leaderboardHref} className="text-sm text-slate-400 hover:text-amber-400 transition-colors">
          ← Leaderboard
        </Link>
      </div>
